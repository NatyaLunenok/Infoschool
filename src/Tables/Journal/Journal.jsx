...
const JournalTable = ({ class: selectedClass, subject: selectedSubject, quarter }) => {
    ...
    // Выносим функцию за пределы useEffect
    const fetchJournalData = async () => {
        try {
            setIsLoading(true);
            setError(null);
            ...
            const [lessonsResponse, marksResponse, homeworksResponse] = await Promise.all([
                FetchWithAuth(`http://127.0.0.1:8000/diary/lessons/?class_id=${selectedClass.id}&subject_id=${selectedSubject.id}&quarter=${quarter}`),
                FetchWithAuth(`http://127.0.0.1:8000/diary/journal/?class_id=${selectedClass.id}&subject_id=${selectedSubject.id}&quarter=${quarter}`),
                FetchWithAuth(`http://127.0.0.1:8000/diary/homeworks/?class_id=${selectedClass.id}&subject_id=${selectedSubject.id}&quarter=${quarter}`)
            ]);

            setLessons(lessonsResponse);

            // Форматирование данных студентов
            const formattedStudents = marksResponse.map(student => ({
                id: student.id,
                student: `${student.last_name} ${student.first_name}`,
                grades: student.grades.reduce((acc, grade) => {
                    const dateKey = formatDate(grade.date);
                    if (!acc[dateKey]) acc[dateKey] = [];
                    acc[dateKey].push({ id: grade.id, mark: grade.mark });
                    return acc;
                }, {})
            }));
            setStudents(formattedStudents);

            // Форматирование домашних заданий
            const formattedHomeworks = homeworksResponse.reduce((acc, hw) => {
                if (hw.homework_id) acc[formatDate(hw.date)] = true;
                return acc;
            }, {});
            setHomeworks(formattedHomeworks);
    ...
    useEffect(() => {
        if (selectedClass && selectedSubject) fetchJournalData();
    }, [selectedClass, selectedSubject, quarter]);

    const handleGradeSubmit = async (studentId, date) => {
        if (!newGrade.trim() || isNaN(newGrade)) return;
        ...
        try {
            ...
            setStudents(prev => prev.map(student => {
                ...
                const updatedGrades = { ...student.grades };
                if (!updatedGrades[date]) updatedGrades[date] = [];
                updatedGrades[date].push({ id: Date.now(), mark: grade }); // Временный ID
    ...
    const handleDeleteGrade = async (gradeId, studentId, date) => {
        try {
            await FetchWithAuth(`http://127.0.0.1:8000/diary/current-marks/${gradeId}/delete/`, {
                method: 'DELETE'
            });
            ...
            setStudents(prev => prev.map(student => {
                ...
                const updatedGrades = { ...student.grades };
                updatedGrades[date] = updatedGrades[date].filter(g => g.id !== gradeId);
    ...
    const handleHomeworkSubmit = async (homework) => {
        try {
            await FetchWithAuth('http://127.0.0.1:8000/diary/homeworks/create/', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    class_id: selectedClass.id,
            ...
            setHomeworks(prev => ({ ...prev, [formatDate(homework.date)]: true }));
            return { success: true, message: 'ДЗ добавлено' };
            
        } catch (error) {
            console.error('Ошибка при добавлении ДЗ:', error);
            return { success: false, message: 'Ошибка добавления ДЗ' };
        }
    };
    ...
    const lessonDates = lessons.map(lesson => formatDate(lesson.date));
                                    ...
                                    <td key={date} onClick={() => {
                                        ...
                                        {isEditing ? (
                                            <input
                                                ...
                                                onBlur={() => handleGradeSubmit(student.id, date)}
                                                onKeyDown={e => {
                                                    if (e.key === 'Enter') handleGradeSubmit(student.id, date);
                                            ...
                                            grades.map((grade, i) => (
                                                <span key={grade.id} style={{ backgroundColor: getGradeColor(grade.mark) }}>
                                                    {grade.mark}
                                                    <button onClick={e => {
                                                        e.stopPropagation();
                                                        handleDeleteGrade(grade.id, student.id, date);
                                                    }}>×</button>
                                                </span>
                                            ))
                                        )}
